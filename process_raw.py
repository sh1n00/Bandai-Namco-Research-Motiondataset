...
from utils import settings, myenum, converter
    ...
    for pos_file in pos_files:
        ...
        for action in myenum.Action.get_all_action():
            if action in pos_file:
                output_folder = os.path.join(settings.ROOT_DIR, "data", "processed", "position", "action", action)
                os.makedirs(output_folder, exist_ok=True)
                df = converter.convert_pos_columns(df)
    ...
    for pos_file in pos_files:
        ...
        for style in myenum.Styles.get_all_styles():
            if style in pos_file:
                output_folder = os.path.join(settings.ROOT_DIR, "data", "processed", "position", "style", style)
                os.makedirs(output_folder, exist_ok=True)
                df = converter.convert_pos_columns(df)
                df.to_csv(os.path.join(output_folder, filename), index=False)
                break
...
def make_master_record_pos():
    ...
    master.to_csv(os.path.join(settings.ROOT_DIR, "data", "processed", "master_pos.csv"), index=False)
...
def make_master_record_rot():
    ...
    for rot_file in rot_files:
        ...
        df["action"] = action
        df["style"] = style
        df = converter.convert_rot_columns(df)
        master = pd.concat([master, df], axis=0)
    master.to_csv(os.path.join(settings.ROOT_DIR, "data", "processed", "master_rot.csv"), index=False)
...
def make_master():
    ...
    for pos_file, rot_file in zip(pos_files, rot_files):
        ...
        df["style"] = style
        master = pd.concat([master, df], axis=0)
    master.to_csv(os.path.join(settings.ROOT_DIR, "data", "processed", "master.csv"), index=False)
